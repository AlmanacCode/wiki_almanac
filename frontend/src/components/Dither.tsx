...
void main() {
  ...
  gl_FragColor = vec4(mix(baseColor, waveColor, f), 1.0);
}
`;

/* ── Bayer dither post-processing effect ─────────────────────────── */
...
>((props, ref) => {
  const Wrapped = wrapEffect(DitherEffectImpl);
...
function DitherScene({
  ...
  mouseRadius,
}: Omit<Required<DitherProps>, "className" | "style">) {
